...
!     layer of the related node
      SUBROUTINE SHARE_DSL()
        ...
        ALLOCATE(smsgbuf(MSGTTMDSL),rmsgbuf(MSGTTMDSL))
        ...
        neigh5: DO nbb = 1,26
           ...
           IF(ncom(nb).EQ.1) THEN 
              ...
              DO icn = 1,nbcell(nb)
                 ...
                 smsgbuf(l + 3) = rden(ic)
                 smsgbuf(l + 4) = conds(ic)
                 smsgbuf(l + 5) = condl(ic)
                 l = l + ITTMDSL
...
!             Define the position of the recieved linkcell
              ...
              rden(ip)  =  rmsgbuf(l + 3)
              conds(ip)  = rmsgbuf(l + 4)
              condl(ip)  = rmsgbuf(l + 5)
              l = l + ITTMDSL
