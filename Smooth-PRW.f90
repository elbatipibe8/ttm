...
333              Neigh: DO nb = 1,13
        ...
        DO ii = 1,ippl
           ip = idlc(ii)
!           IF(R_a(ip).EQ.0.0d0) THEN
!              TPSX(ip) = 0.0d0
!           ELSE
              TPSX(ip)=P_a(ip)/R_a(ip)
!!           ENDIF
!           IF(dcount(ip).EQ.0.0d0) THEN
!              RON(ip) = 0.0d0
!           ELSE
              RON(ip) = R_a(ip)/dcount(ip)
!           ENDIF
        ENDDO
